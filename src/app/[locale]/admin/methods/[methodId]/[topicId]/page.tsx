...
import {
    ...
    Upload, HelpCircle, Loader2, Layout, Sparkles, FileText, BrainCircuit
} from "lucide-react"
...
import { getMethodTopicDetail, updateMethodTopic, addMethodTopic, getMethodTopics, uploadMindmap } from "@/services/firestore"
...
export default function MethodTopicDetailPage() {
    ...
    useEffect(() => {
        if (methodId && topicId) loadTopicDetail()
    }, [methodId, topicId])
    ...
    const loadTopicDetail = async () => {
        if (topicId === 'new') {
            setIsLoading(false)
            try {
                const existingTopics = await getMethodTopics(methodId as string)
                setTopic({ order: existingTopics.length + 1 } as MethodTopic)
            } catch (error) {
                console.error("Error fetching topics for order:", error)
                setTopic({ order: 1 } as MethodTopic)
            }
            return
        }

        try {
            const data = await getMethodTopicDetail(methodId as string, topicId as string)
            if (data) {
                setTopic(data)
                setTitle(data.title)
            ...
            console.error("Error loading method topic detail:", error)
        } finally {
            setIsLoading(false)
        }
    }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (topicId === 'new') {
                await addMethodTopic(methodId as string, {
                    ...topicData,
                    order: topic?.order || 1
                })
            } else {
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-40 gap-4">
                ...
                <p className="text-slate-500 font-medium">Dars tafsilotlari yuklanmoqda...</p>
            </div>
        )
    }
        ...
        <div className="max-w-7xl mx-auto space-y-8 pb-20">
            ...
            <div className="flex items-center justify-between sticky top-0 z-50 bg-white/80 backdrop-blur-md py-4 border-b border-slate-100">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-900 truncate max-w-[300px]">{title || "Nomsiz dars"}</h1>
                        <p className="text-xs text-slate-500 uppercase tracking-widest font-semibold">Dars muharriri</p>
                    </div>
                </div>
                <Button
                    className="bg-amber-600 hover:bg-amber-700 text-white rounded-xl px-6 shadow-lg shadow-amber-600/20 gap-2 font-bold transition-all"
                    onClick={handleSave}
                    disabled={isSaving}
                >
            ...
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                    ...
                    <CardContent className="p-6">
                        <Input
                            ...
                            className="h-12 text-lg font-semibold border-slate-200 rounded-xl focus:ring-amber-500"
                        />
                    </CardContent>
                </Card>
                ...
                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                    <CardHeader className="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <CardDescription>Dars yuzasidan barcha nazariy ma'lumotlar.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6">
                ...
                {/* First Principles */}
                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl bg-amber-50/30 border-amber-100">
                    <CardHeader className="border-b border-amber-100">
                        <CardTitle className="text-lg flex items-center gap-2 text-amber-900">
                            <Sparkles className="w-5 h-5 text-amber-500" />
                            First Principles (Metodika)
                        </CardTitle>
                        <CardDescription className="text-amber-700/70">Chuqurlashtirilgan tahlil bloki.</CardDescription>
                    </CardHeader>
                    <CardContent className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <CardContent className="p-6">
                            {videoUrl ? (
                                ...
                                <div
                                    ...
                                    onClick={() => document.getElementById('method-video-upload')?.click()}
                                >
                                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm mb-3">
                                        {uploadingMedia === 'video' ? <Loader2 className="w-6 h-6 animate-spin text-amber-600" /> : <Upload className="w-6 h-6 text-slate-400" />}
                                    </div>
                                    <p className="text-sm font-bold text-slate-600">Video yuklash</p>
                                    <input id="method-video-upload" type="file" className="hidden" accept="video/*" onChange={(e) => handleFileUpload(e, 'video')} />
                                </div>
                            )}
                        </CardContent>
                    </Card>
                    ...
                    <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                        ...
                        <CardContent className="p-6">
                            {mindmapUrl ? (
                                <div className="space-y-3">
                                    <div className="aspect-auto min-h-[200px] bg-slate-100 rounded-xl border border-slate-200 overflow-hidden relative group">
                                ...
                                <div
                                    className="aspect-video border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50/50 hover:bg-slate-100 transition-all cursor-pointer"
                                    onClick={() => document.getElementById('method-mindmap-upload')?.click()}
                                >
                                    <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center shadow-sm mb-3">
                                        <BrainCircuit className="w-6 h-6 text-slate-400" />
                                    </div>
                                    <p className="text-sm font-bold text-slate-600">Mindmap yuklash</p>
                                    <input id="method-mindmap-upload" type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'mindmap')} />
                                </div>
                            )}
                        </CardContent>
                    </Card>
                ...
                <Card className="border-slate-200 shadow-sm overflow-hidden rounded-2xl">
                    <CardHeader className="bg-slate-50/50 border-b border-slate-100 flex flex-row items-center justify-between">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={() => handleAddQuiz('closed')} className="rounded-lg border-amber-200 text-amber-700 hover:bg-amber-50">
                                + Yopiq test
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => handleAddQuiz('open')} className="rounded-lg border-teal-200 text-teal-700 hover:bg-teal-50">
                                + Ochiq savol
                            </Button>
                        </div>
                    </CardHeader>
                            ...
                            quizzes.map((quiz, idx) => (
                                <div key={quiz.id} className="p-6 rounded-2xl border border-slate-100 bg-slate-50/50 relative group">
                                    ...
                                    <div className="flex items-center gap-3 mb-4">
                                        <Badge className={cn(
                                            "border-none px-3",
                                            quiz.type === 'closed' ? "bg-amber-100 text-amber-700" : "bg-teal-100 text-teal-700"
                                        )}>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        {quiz.type === 'open' && (
                                            <div className="p-4 bg-teal-50 rounded-xl border border-teal-100">
                                                <p className="text-[10px] text-teal-700 font-bold uppercase mb-1">AI Tekshiruv</p>
                                                <p className="text-xs text-teal-600">Ushbu savol javobi sun'iy intellekt tomonidan tahlil qilinadi.</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
